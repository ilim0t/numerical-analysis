...
TEST(Data_Mat_Definition, Uniform_Initialization1) {
    EXPECT_NO_THROW(({
        Mat<int, 2, 2> mat{
...
TEST(Data_Mat_Definition, Uniform_Initialization2) {
    ...
    EXPECT_EQ(
            (Mat<std::complex<double>, 2, 2>{
                    ...
                    .at(1).at(0)), std::complex<double>(-0.74, -0.89));
}

TEST(Data_Mat_Definition, Initialization1) {
...
TEST(Data_Mat_Definition, Shape) {
...
TEST(Data_Mat_Definition, RareShape1) {
    ...
    EXPECT_EQ((Mat<int, 1, 1>{{123}}.at(0).at(0)), 123);
}
...
TEST(Data_Mat_Definition, RareShape2) {
    ...
    EXPECT_EQ((Mat<int, 1, 2>{{100, 101}}.at(0).at(0)), 100);
    EXPECT_EQ((Mat<int, 1, 2>{{200, 201}}.at(0).at(1)), 201);
...
TEST(Data_Mat_Definition, InvalidShape) {
    ASSERT_ANY_THROW(({
        Mat<int, 0, 0> mat(0);
    }));
    ASSERT_ANY_THROW(({
